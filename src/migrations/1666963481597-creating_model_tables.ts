...
export class creatingModelTables1666963481597 implements MigrationInterface {
  name = 'creatingModelTables1666963481597';

  public async up(queryRunner: QueryRunner): Promise<void> {
    await queryRunner.query(
      `CREATE TABLE \`medication_entity\` (\`id\` int NOT NULL AUTO_INCREMENT, \`name\` varchar(255) NOT NULL, \`weight\` int NOT NULL, \`code\` varchar(255) NOT NULL, \`image\` varchar(255) NOT NULL, \`droneHostId\` int NULL, PRIMARY KEY (\`id\`)) ENGINE=InnoDB`,
    );
    await queryRunner.query(
      `CREATE TABLE \`drone_entity\` (\`id\` int NOT NULL AUTO_INCREMENT, \`serial\` varchar(255) NOT NULL, \`model\` enum('Lightweight', 'Middleweight', 'Cruiserweight', 'Heavyweight') NOT NULL, \`weight_limit\` int NOT NULL, \`available_weight\` int NOT NULL, \`battery_capacity\` int NOT NULL, \`state\` enum('IDLE', 'LOADING', 'LOADED', 'DELIVERING', 'DELIVERED', 'RETURNING') NOT NULL, UNIQUE INDEX \`IDX_5095a7eed83f3836ea29139f48\` (\`serial\`), PRIMARY KEY (\`id\`)) ENGINE=InnoDB`,
    );
    await queryRunner.query(
